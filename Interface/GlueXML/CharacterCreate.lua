...
local function UpdateClassButtonEnabledState(button, classID, classData)
	...
	if ( classData.enabled == true or classData.disableReason == LE_DEMON_HUNTER_INVALID_CLASS_FOR_BOOST or (GetCharacterCreateType() == LE_CHARACTER_CREATE_TYPE_TRIAL_BOOST and classData.disableReason == LE_DEMON_HUNTER_CREATION_DISABLED_REASON_NEED_LEVEL_70)) then
		...
		disableTexture:Show();
        -- deselect demon hunter and select warrior when switching to an invalid race/demon hunter combination
        -- or when selecting normal non trial char and player doesn't meet demon hunter requirements
        local _,_,currClass = GetSelectedClass();
        if (classID == 12 and currClass == 12) then
            SetSelectedClass(1);
            SetCharacterClass(1);
            -- force customization update
            SetCharacterGender(GetSelectedSex());
            CharacterChangeFixup();
            RandomizeCharCustomization(true);
            if ( CharacterCreateFrame.state == "CUSTOMIZATION" ) then
...
local classTypeData = {
	...
	["DEMONHUNTER"] = { startingLevel = 98, allowBoost = true, },
